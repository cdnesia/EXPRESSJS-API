...
const TEMPLATE_PATH = path.join(__dirname, '../views/pdf/krs.ejs');
const LOGO_PATH = path.join(__dirname, '../../public/assets/images/favicon-32x32.png');

const cetakKrsSchema = z
  .object({
...
async function cetak(req, res, next) {
  try {
    const { npm, periode, view = 'download' } = parseOrThrow(cetakKrsSchema, req.body);

    const [saya, krs] = await Promise.all([akademikService.getStudent(npm), akademikService.getKrs(npm, periode)]);
    ...
    const filename = `KRS_${safeNpm}_${periode}.pdf`;
